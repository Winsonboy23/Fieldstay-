// Internal API：後台確認活動匯款後呼叫，寄送「匯款已確認」通知信給報名者
...
import { activityPaidEmail } from "@/app/_lib/emailTemplates";
...
export async function POST(request) {
  ...
  if (error || !signup) {
    console.error("activity-paid lookup failed", error);
    return json({ error: "not_found" }, { status: 404 });
  }

  if (signup.paid_email_sent_at) {
  ...
  const { subject, html } = activityPaidEmail({
    signup: {
      id: signup.id,
