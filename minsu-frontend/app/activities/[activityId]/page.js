    ...
    <main className="min-h-screen bg-[#f5f3ef] text-[#111827]">
      <header className="border-b border-[#e4dfd8] bg-white">
        <div className="mx-auto flex h-16 max-w-[1280px] items-center gap-3 px-6">
          <Link href="/activities" className="grid h-9 w-9 place-items-center rounded-full bg-[#007d6f] text-white">
      ...
      <div className="mx-auto grid max-w-[1280px] gap-8 px-6 py-10 lg:grid-cols-[1fr_390px]">
        <section>
          <div className={`relative mb-7 h-[390px] overflow-hidden rounded-2xl bg-gradient-to-br ${activity.heroClass}`}>
            <span className="absolute left-5 top-5 rounded-md bg-white px-3 py-1 text-xs font-bold text-[#007d6f]">
              {activity.category}
            </span>
            <button className="absolute right-5 top-5 grid h-11 w-11 place-items-center rounded-full bg-white text-slate-600" aria-label="加入收藏">
          ...
          <div className="mb-7 flex gap-4">
            <div className="w-16 shrink-0 text-center">
              <div className="font-serif text-5xl font-black leading-none">{activity.day}</div>
            ...
            <div className="space-y-6 text-slate-700">
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p>{activity.dateLabel} {activity.time}</p>
                </div>
              </div>
              <div className="flex gap-4">
        ...
        <aside className="lg:sticky lg:top-8 lg:self-start">
          <div className="rounded-2xl border border-[#ddd7cf] bg-white p-6 shadow-sm">
            <div className="mb-2 font-serif text-4xl font-black">
              {formatPrice(activity.price)} <span className="font-sans text-lg font-normal text-slate-600">/ {activity.unit}</span>
            </div>
            <p className={isFull ? "mb-7 font-bold text-slate-500" : "mb-7 font-bold text-red-600"}>
              {isFull ? "目前已額滿" : `剩餘 ${remaining} 名`}
            </p>
            <div className="mb-7 border-y border-[#e5dfd8] py-6 text-sm">
              <div className="mb-4 flex justify-between gap-5"><span className="text-slate-500">活動日期</span><strong>{activity.dateLabel}</strong></div>
              <div className="mb-4 flex justify-between gap-5"><span className="text-slate-500">活動時間</span><strong>{activity.time}</strong></div>
              <div className="mb-4 flex justify-between gap-5"><span className="text-slate-500">活動時長</span><strong>{activity.duration}</strong></div>
              <div className="flex justify-between gap-5"><span className="text-slate-500">已報名</span><strong>{activity.registered} / {activity.capacity} 人</strong></div>
            </div>
            <Link
            ...
            <p className="mt-6 text-center text-sm text-slate-500">報名後將收到確認郵件</p>
          </div>
        </aside>
      </div>
    </main>
