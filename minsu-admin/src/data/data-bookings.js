import { add } from 'date-fns';

function fromToday(numDays, withTime = false) {
  const date = add(new Date(), { days: numDays });
  if (!withTime) date.setUTCHours(0, 0, 0, 0);
  return date.toISOString().slice(0, -1);
}

export const bookings = [
  // ROOM 001
  {
    created_at: fromToday(-20, true),
    startDate: fromToday(0),
    endDate: fromToday(7),
    roomId: 1,
    guestId: 2,
    hasBreakfast: true,
    observations:
      "Please arrange an early check-in if possible. My bus from Chattogram reaches Dhaka in the morning.",
    isPaid: false,
    numGuests: 1,
  },
  {
    created_at: fromToday(-33, true),
    startDate: fromToday(-23),
    endDate: fromToday(-13),
    roomId: 1,
    guestId: 3,
    hasBreakfast: true,
    observations: '',
    isPaid: true,
    numGuests: 2,
  },
  {
    created_at: fromToday(-27, true),
    startDate: fromToday(12),
    endDate: fromToday(18),
    roomId: 1,
    guestId: 4,
    hasBreakfast: false,
    observations: "We will arrive after Isha prayer, around 9:30 PM.",
    isPaid: false,
    numGuests: 2,
  },

  // ROOM 002
  {
    created_at: fromToday(-45, true),
    startDate: fromToday(-45),
    endDate: fromToday(-29),
    roomId: 2,
    guestId: 5,
    hasBreakfast: false,
    observations: '',
    isPaid: true,
    numGuests: 2,
  },
  {
    created_at: fromToday(-2, true),
    startDate: fromToday(15),
    endDate: fromToday(18),
    roomId: 2,
    guestId: 6,
    hasBreakfast: true,
    observations: "Please keep a prayer mat in the room.",
    isPaid: true,
    numGuests: 2,
  },
  {
    created_at: fromToday(-5, true),
    startDate: fromToday(33),
    endDate: fromToday(48),
    roomId: 2,
    guestId: 7,
    hasBreakfast: true,
    observations: "Airport pickup from Hazrat Shahjalal International Airport would be helpful.",
    isPaid: false,
    numGuests: 2,
  },

  // ROOM 003
  {
    created_at: fromToday(-65, true),
    startDate: fromToday(-25),
    endDate: fromToday(-20),
    roomId: 3,
    guestId: 8,
    hasBreakfast: true,
    observations: '',
    isPaid: true,
    numGuests: 4,
  },
  {
    created_at: fromToday(-2, true),
    startDate: fromToday(-2),
    endDate: fromToday(0),
    roomId: 3,
    guestId: 9,
    hasBreakfast: false,
    observations: "We are traveling with children and would like adjacent beds if possible.",
    isPaid: true,
    numGuests: 3,
  },
  {
    created_at: fromToday(-14, true),
    startDate: fromToday(-14),
    endDate: fromToday(-11),
    roomId: 3,
    guestId: 10,
    hasBreakfast: true,
    observations: '',
    isPaid: true,
    numGuests: 4,
  },

  // ROOM 004
  {
    created_at: fromToday(-30, true),
    startDate: fromToday(-4),
    endDate: fromToday(8),
    roomId: 4,
    guestId: 11,
    hasBreakfast: true,
    observations: "We are attending a wedding in Dhanmondi and may need late-night entry.",
    isPaid: true,
    numGuests: 4,
  },
  {
    created_at: fromToday(-1, true),
    startDate: fromToday(12),
    endDate: fromToday(17),
    roomId: 4,
    guestId: 12,
    hasBreakfast: true,
    observations: "Please include tea and paratha options at breakfast.",
    isPaid: false,
    numGuests: 4,
  },
  {
    created_at: fromToday(-3, true),
    startDate: fromToday(18),
    endDate: fromToday(19),
    roomId: 4,
    guestId: 13,
    hasBreakfast: false,
    observations: '',
    isPaid: true,
    numGuests: 1,
  },

  // ROOM 005
  {
    created_at: fromToday(0, true),
    startDate: fromToday(14),
    endDate: fromToday(21),
    roomId: 5,
    guestId: 14,
    hasBreakfast: true,
    observations: "Our family is coming from Sylhet; please keep extra towels ready.",
    isPaid: false,
    numGuests: 5,
  },
  {
    created_at: fromToday(-6, true),
    startDate: fromToday(-6),
    endDate: fromToday(-4),
    roomId: 5,
    guestId: 15,
    hasBreakfast: true,
    observations: '',
    isPaid: true,
    numGuests: 4,
  },
  {
    created_at: fromToday(-4, true),
    startDate: fromToday(-4),
    endDate: fromToday(-1),
    roomId: 5,
    guestId: 16,
    hasBreakfast: false,
    observations: "We may need help arranging transport to Sadarghat.",
    isPaid: true,
    numGuests: 6,
  },

  // ROOM 006
  {
    created_at: fromToday(-3, true),
    startDate: fromToday(0),
    endDate: fromToday(11),
    roomId: 6,
    guestId: 17,
    hasBreakfast: false,
    observations:
      "We will check in late after landing at Dhaka airport, around midnight.",
    isPaid: true,
    numGuests: 6,
  },
  {
    created_at: fromToday(-16, true),
    startDate: fromToday(-16),
    endDate: fromToday(-9),
    roomId: 6,
    guestId: 18,
    hasBreakfast: true,
    observations: "I will need a rollaway bed for one of the guests.",
    isPaid: true,
    numGuests: 4,
  },
  {
    created_at: fromToday(-18, true),
    startDate: fromToday(-4),
    endDate: fromToday(-1),
    roomId: 6,
    guestId: 19,
    hasBreakfast: true,
    observations: '',
    isPaid: true,
    numGuests: 6,
  },

  // ROOM 007
  {
    created_at: fromToday(-2, true),
    startDate: fromToday(17),
    endDate: fromToday(23),
    roomId: 7,
    guestId: 20,
    hasBreakfast: false,
    observations: "We are starting our Cox's Bazar trip from Dhaka and need parking for one microbus.",
    isPaid: false,
    numGuests: 8,
  },
  {
    created_at: fromToday(-7, true),
    startDate: fromToday(40),
    endDate: fromToday(50),
    roomId: 7,
    guestId: 21,
    hasBreakfast: true,
    observations: "Please arrange breakfast early; our group leaves for Srimangal at 7 AM.",
    isPaid: true,
    numGuests: 7,
  },
  {
    created_at: fromToday(-55, true),
    startDate: fromToday(32),
    endDate: fromToday(37),
    roomId: 7,
    guestId: 22,
    hasBreakfast: true,
    observations: '',
    isPaid: true,
    numGuests: 6,
  },

  // ROOM 008
  {
    created_at: fromToday(-8, true),
    startDate: fromToday(-5),
    endDate: fromToday(0),
    roomId: 8,
    guestId: 1,
    hasBreakfast: true,
    observations:
      "My parents prefer less spicy food at breakfast if possible.",
    isPaid: true,
    numGuests: 9,
  },
  {
    created_at: fromToday(0, true),
    startDate: fromToday(0),
    endDate: fromToday(5),
    roomId: 8,
    guestId: 23,
    hasBreakfast: true,
    observations:
      "We are celebrating our anniversary; simple flower decoration would be appreciated.",
    isPaid: true,
    numGuests: 10,
  },
  {
    created_at: fromToday(-10, true),
    startDate: fromToday(10),
    endDate: fromToday(13),
    roomId: 8,
    guestId: 24,
    hasBreakfast: false,
    observations: '',
    isPaid: true,
    numGuests: 7,
  },
];
